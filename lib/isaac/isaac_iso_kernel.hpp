    ...
    struct IsoStepRenderKernel
    {
        ...
        ) const
        {
            ...
            for(isaac_int i = startSteps; i <= endSteps && !hit; i++)
            {
                ...
                forEachWithMplParams(
                    sources,
                    IsoStepSourceIterator<T_transferSize, T_Filter, T_interpolation, T_isoSurface>(),
                    ray,
                    t,
                    pos,
                    stepSize,
...
#define ISAAC_KERNEL_START                                                                                            \
    {                                                                                                                 \
        IsoStepRenderKernel < T_SourceList, T_TransferArray, T_IsoThreshold, T_PointerArray, T_Filter, T_transferSize,
#define ISAAC_KERNEL_END                                                                                              \
    ...
    alpaka::enqueue(stream, instance);                                                                                \
            ...
            if(interpolation)
            {
                ISAAC_KERNEL_START 1, 1 ISAAC_KERNEL_END
            }
            else
            {
                ISAAC_KERNEL_START 0, 1 ISAAC_KERNEL_END
            }
