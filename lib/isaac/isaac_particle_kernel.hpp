...
namespace isaac
{
     ...
     * @tparam T_Filter
     */
    template<ISAAC_IDX_TYPE T_transferSize, int T_offset, typename T_Filter>
    struct MergeParticleSourceIterator
    {
        template<typename T_NR, typename T_Source, typename T_TransferArray, typename T_SourceWeight>
            ...
            if(boost::mpl::at_c<T_Filter, T_NR::value>::type::value)
            {
                ...
                for(int i = 0; i < particleIterator.size; ++i)
                {
                    ...
                    isaac_float3 particlePos
                        = (particleIterator.getPosition() + isaac_float3(cellPos)) * particleScale;
                    isaac_float3 L = particlePos - start;
                    isaac_float radius = particleIterator.getRadius() * sourceWeight.value[T_NR::value + T_offset];
                    ...
                    if(d2 <= radius2)
                    {
                        ...
                        {
                            isaac_float_dim<T_Source::featureDim> data = particleIterator.getAttribute();
    ...
    struct ParticleRenderKernelCaller
    {
        ...
        {
            if(sourceWeight.value[T_sourceOffset + boost::mpl::size<T_ParticleList>::type::value - T_n]
               == isaac_float(0))
            {
                ParticleRenderKernelCaller<
                    ...
                    T_sourceOffset,
                    T_n - 1>::
                    call(stream, gBuffer, particleSources, transferArray, sourceWeight, workdiv, scale, clipping);
            }
            else
            {
                ParticleRenderKernelCaller<
