...
// new memory access test
    ...
    ISAAC_DEVICE_INLINE void checkCoord(isaac_float3& coord, const isaac_size3& localSize)
    {
        if(T_Source::hasGuard || !T_Source::persistent)
        {
            coord = glm::clamp(
    ...
    template<ISAAC_IDX_TYPE T_transferSize, typename T_Filter, isaac_int T_interpolation, isaac_int T_isoSurface>
            ...
            isaac_float3& normal) const
        {
            if(boost::mpl::at_c<T_Filter, T_NR::value>::type::value)
            {
                ...
                if(T_isoSurface)
                {
                    ...
                    {
                        isaac_float3 gradient
                            = getGradient<T_interpolation, T_NR>(source, pos, pointerArray, localSize);
    ...
    struct VolumeRenderKernel
    {
        ...
        ) const
        {
            ...
            isaac_float min_size = glm::min(
                SimulationSize.globalSize.x,
                glm::min(SimulationSize.globalSize.y, SimulationSize.globalSize.z));
            ...
            for(isaac_int i = startSteps; i <= endSteps; i++)
            {
                ...
                forEachWithMplParams(
                    sources,
                    MergeVolumeSourceIterator<T_transferSize, T_Filter, T_interpolation, T_isoSurface>(),
                    pos,
            ...
            //    || !isInUpperBounds(startUnscaled + stepVec * isaac_float(endSteps), isaac_float3(
            //    SimulationSize.localSize ))) color = isaac_float4(1, 1, 1, 1);
...
#define ISAAC_KERNEL_START                                                                                            \
    {                                                                                                                 \
        VolumeRenderKernel < T_SourceList, T_TransferArray, T_SourceWeight, T_PointerArray, T_Filter, T_transferSize,
#define ISAAC_KERNEL_END                                                                                              \
    ...
    alpaka::enqueue(stream, instance);                                                                                \
            ...
            if(interpolation)
            {
                ISAAC_KERNEL_START 1, 0 ISAAC_KERNEL_END
            }
            else
            {
                ISAAC_KERNEL_START 0, 0 ISAAC_KERNEL_END
            }
