...
namespace isaac
{
    ...
    class Texture
    {
        public:
            ...
            void init(T_Type* bufferPtr, const isaac_size_dim<T_textureDim> & size)
            {
            ...
            T_Type sample( 
                ...
                const T_Type & borderValue = T_Type( 0 )
            )
            {
                T_Type result;
            ...
            ISAAC_HOST_DEVICE_INLINE
            T_Type operator[](const isaac_int_dim<T_textureDim>& coord) const
            {
            ...
            ISAAC_HOST_DEVICE_INLINE
            T_Type & operator[](const isaac_int_dim<T_textureDim>& coord)
            {
                isaac_uint_dim<T_textureDim> offsetCoord = coord + isaac_int( T_guardSize );
                return bufferPtr[hash( offsetCoord )];
            }
            ...
            )
            {
                ...
                {
                    offsetCoord = glm::clamp( coord + isaac_int( T_guardSize ), isaac_int_dim<T_textureDim>(0), isaac_int_dim<T_textureDim>( sizeWithGuard ) );
                }
            ...
            )
            {
                ...
                for( int x = 0; x < 2; x++ )
                {
                    for( int y = 0; y < 2; y++ )
                    {
                        for( int z = 0; z < 2; z++ )
                        {
                            data8[x][y][z] = safeMemoryAccess<T_border>( isaac_int3( coord ) + isaac_int3( x, y, z ), borderValue );
                        }
                    }
                }
