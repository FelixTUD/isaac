...
public:
    ...
    TestSource1(isaac_float3* ptr) : ptr(ptr)
    {
    }
    ...
    ISAAC_DEVICE_INLINE isaac_float_dim<featureDim> operator[](const isaac_int3& nIndex) const
    {
...
public:
    ...
    TestSource2(isaac_float* ptr) : ptr(ptr)
    {
    }
...
public:
    ...
    ISAAC_DEVICE_INLINE ParticleIterator1(T_ElemType* firstElement, size_t size, const isaac_uint3& localGridCoord)
    ...
    ISAAC_DEVICE_INLINE void next()
    {
        currentElement = &currentElement[1];
    }


    ISAAC_DEVICE_INLINE T_ElemType getPosition() const
    {
        return *currentElement;
    }


    ISAAC_DEVICE_INLINE isaac_float_dim<T_featureDim> getAttribute() const
    {
        return {isaac_float(localGridCoord.x), isaac_float(localGridCoord.y), isaac_float(localGridCoord.z)};
    }


    ISAAC_DEVICE_INLINE isaac_float getRadius() const
    {
        return 0.05f;
    }
...
public:
    ...
    ParticleSource1(isaac_float3* ptr, size_t size) : ptr(ptr), size(size)
    {
    }
    ...
    ISAAC_DEVICE_INLINE
    ParticleIterator1<featureDim, isaac_float3> getIterator(const isaac_uint3& localGridCoord) const
    {
        return ParticleIterator1<featureDim, isaac_float3>(ptr, size, localGridCoord);
    }
};
...
int main(int argc, char** argv)
{
    ...
    auto hostBufferNoise = alpaka::allocBuf<isaac_float, ISAAC_IDX_TYPE>(devHost, dataSize);
    auto hostBufferLIC = alpaka::allocBuf<isaac_float, ISAAC_IDX_TYPE>(devHost, dataSize);
    auto deviceBufferLIC = alpaka::allocBuf<isaac_float, ISAAC_IDX_TYPE>(devAcc, dataSize);
    ...
    TestSource2 testSourceNoise(alpaka::getPtrNative(deviceBufferLIC));

    ParticleSource1 particleTestSource1(alpaka::getPtrNative(deviceBuffer3), PARTICLE_COUNT);

    using SourceList = boost::fusion::list<TestSource1, TestSource2, TestSource2>;

    using ParticleList = boost::fusion::list<ParticleSource1>;

    ParticleList particleSources(particleTestSource1);
    SourceList sources(testSource1, testSource2, testSourceNoise);
    ...
    for(size_t x = 0; x < localSize.x; x++)
    {
        for(size_t y = 0; y < localSize.y; y++)
        {
            ...
            {
                size_t pos = x + y * localSize.x + z * localSize.x * localSize.y;
        ...
        for(int y = 0; y < localSize.y; y++)
        {
            for(int z = 0; z < localSize.z; z++)
            {
                ...
                for(int i = -2; i < 3; i++)
                {
                    ...
                    size_t pos = coord.x + coord.y * localSize.x + coord.z * localSize.x * localSize.y;
                ...
                size_t oldPos = x + y * localSize.x + z * localSize.x * localSize.y;
        ...
        for(size_t y = 0; y < localSize.y; y++)
        {
            for(size_t z = 0; z < localSize.z; z++)
            {
                ...
                for(int i = 0; i < steps; i++)
                {
                    ...
                    coord += tangent;
                    size_t pos = ISAAC_IDX_TYPE(coord.x) + ISAAC_IDX_TYPE(coord.y) * localSize.x
                        + ISAAC_IDX_TYPE(coord.z) * localSize.x * localSize.y;
    ...
    alpaka::memcpy(stream, deviceBufferLIC, hostBufferNoise, prod);
    ...
    while(!forceExit)
    {
        ...
        if(step >= interval)
        {
            ...
            if(rank == 0)
            {
                ...
                json_object_set_new(visualization->getJsonMetaRoot(), "drawing_time", json_integer(drawingTime));
                json_object_set_new(visualization->getJsonMetaRoot(), "simulation_time", json_integer(simulationTime));
                json_object_set_new(
